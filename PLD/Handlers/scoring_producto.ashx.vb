...
Public Class scoring_producto : Implements IHttpHandler
        ...
        Select Case op
            Case "consulta"
                ...
                Using conn As New SqlConnection(ConfigurationManager.ConnectionStrings("PLDConnection").ConnectionString)
                    conn.Open()
                    Dim cmd As New SqlCommand("SELECT * FROM scoring_producto_credito ORDER BY id", conn)
                    ...
                    While rd.Read()
                        lista.Add(New With {
                          ...
                          .opcion = rd("opcion"),
                          .minimo_antiguedad_meses = rd("minimo_antiguedad_meses"),
                          .sueldo_x = rd("sueldo_x"),
                          .monto_min_configurado = rd("monto_min_configurado"),
                          .monto_max_configurado = rd("monto_max_configurado")
                        })
                    End While
                End Using
            ...
            Case "guardar"
                ...
                Dim camposValidos = {"producto", "opcion", "minimo_antiguedad_meses", "sueldo_x", "monto_min_configurado", "monto_max_configurado"}
                ...
                Using conn As New SqlConnection(ConfigurationManager.ConnectionStrings("PLDConnection").ConnectionString)
                    conn.Open()
                    Dim cmd As New SqlCommand($"UPDATE scoring_producto_credito SET {campo} = @valor, fecha_actualizacion = GETDATE() WHERE id = @id", conn)
