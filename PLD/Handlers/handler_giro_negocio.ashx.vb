...
Public Class handler_giro_negocio : Implements IHttpHandler
    ...
    Public Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        Try
            Using conn As New SqlConnection(connStr)
                ...
                Select Case action
                    Case "listar"
                        Dim query As String = "SELECT * FROM catalogo_giro_negocio ORDER BY descripcion"
                    ...
                    Case "guardar"
                        ...
                        Dim query As String = "INSERT INTO catalogo_giro_negocio (descripcion, clasificacion_riesgo, impacto, probabilidad, nivel_riesgo_pld, activo, fecha_creacion) VALUES (@descripcion, @clasificacion, @impacto, @probabilidad, @nivelRiesgo, @activo, GETDATE())"
                        Using cmd As New SqlCommand(query, conn)
                    ...
                    Case "editar"
                        ...
                        Dim query As String = "UPDATE catalogo_giro_negocio SET descripcion=@descripcion, clasificacion_riesgo=@clasificacion, impacto=@impacto, probabilidad=@probabilidad, nivel_riesgo_pld=@nivelRiesgo, activo=@activo WHERE id=@id"
                        Using cmd As New SqlCommand(query, conn)
                            cmd.Parameters.AddWithValue("@descripcion", descripcion)
                            cmd.Parameters.AddWithValue("@clasificacion", clasificacion)
                            cmd.Parameters.AddWithValue("@impacto", impacto)
                    ...
                    Case "eliminar"
                        Dim id As Integer = Convert.ToInt32(context.Request("id"))
                        Dim query As String = "DELETE FROM catalogo_giro_negocio WHERE id = @id"
                        Using cmd As New SqlCommand(query, conn)
                            cmd.Parameters.AddWithValue("@id", id)
                            cmd.ExecuteNonQuery()
                        End Using
