...
Public Class scoring_pago : Implements IHttpHandler
        ...
        Select Case op
            Case "consulta"
                ...
                Using conn As New SqlConnection(ConfigurationManager.ConnectionStrings("PLDConnection").ConnectionString)
                    conn.Open()
                    Dim cmd As New SqlCommand("SELECT id, etiqueta, valor FROM scoring_capacidad_pago ORDER BY id", conn)
            ...
            Case "guardar"
                ...
                Using conn As New SqlConnection(ConfigurationManager.ConnectionStrings("PLDConnection").ConnectionString)
                    conn.Open()
                    Dim cmd As New SqlCommand("UPDATE scoring_capacidad_pago SET valor = @valor, fecha_actualizacion = GETDATE() WHERE id = @id", conn)
